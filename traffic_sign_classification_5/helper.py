...
def load_data(filename):
    ...
    print(filename + " length: {}".format(len(data['features'])))
...
def augment_dataset(features, labels, n_classes):
    ...
    print("x: {}, y: {}".format(len(augmented_features), len(augmented_labels)))
    return augmented_features, augmented_labels


#
#
# def augment_dataset_from_disk(features, labels, folder, extension='*.png'):
#     import numpy as np
#     import os
#     import glob
#     from PIL import Image
#     from matplotlib import pyplot as plt
#     images = glob.glob(os.getcwd() + folder + extension)
#
#     # transform to numpy array
#     # im = np.array(im)
#     # # get the class from the filename
#     label = filename.split('/')[-1].split('-')[0]
#     # # grow the features and labels with new images and labels
#     # features = np.concatenate((features, [im]), axis=0)
#     # labels = np.append(labels, label)
#     # print("x: {}, y: {}".format(len(features), len(labels)))
#     return features, labels
#
